...
const char *SSID = "iPhone Lui";     // WiFi SSID
const char *PWD = "Tamarindos"; // WiFi password
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (!strcmp(topic, "Calibration"))
  {
    ...
    // calibration_temperature = incoming;
    // calibration_delta = calibration(incoming);
...
if (digitalRead(buttonPin) == HIGH) {
  ...
  strcat(foo2, foo3);
  strcat(foo2,", \"temp\": ");
  sprintf(foo3, "%.2f", temp_c);
  strcat(foo2, foo3);
  strcat(foo2,", \"occupancy\": ");
  sprintf(foo3, "%d", occupancy);
  strcat(foo2, foo3);
  strcat(foo2," } ");

  mqttClient.publish("Foo", foo2);
